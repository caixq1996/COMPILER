...
class Dset(object):
    ...
    def get_next_batch(self, batch_size):
        ...
        self.pointer = end
        return np.squeeze(inputs), np.squeeze(labels)

class Mujoco_Dset(object):
    def __init__(self, expert_path, train_fraction=0.8, ret_threshold=None, traj_limitation=np.inf, randomize=True):
        ...
        # print('Raw acs:', len(acs), len(acs[0]), acs[0][0].shape)
        # print('Before acs:', self.acs.shape)
        # print('obs: ', self.obs.shape)
        if len(self.acs) > 2:
            self.acs = np.squeeze(self.acs)
        # print('After acs:', self.acs.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--expert_path", type=str, default="../baselines/deterministic.ppo.PongNoFrameskip.0.01.pkl")
