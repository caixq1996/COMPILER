...
def get_distance_curve(
    ...
    for lambda_value in lambda_values:
        ...
        prev_soln = solution
        if distance_sqd < 0:
            sys.stdout.write("[Warning] distance is {:.4f}".format(distance_sqd))
            sys.stdout.flush()
            distance_sqd = 0
...
def mpe(kernel, N, M, nu, epsilon=0.04, lambda_upper_bound=8.):
    ...
    while lambda_right - lambda_left > epsilon:
        # sys.stdout.write('Left-Right: {:.2f}, epsilon: {:.2f}'.format(lambda_right - lambda_left, epsilon))
...
def wrapper(X_mixture, X_component, num_mix, num_com, dim=1):
    """ Takes in 2 arrays containing the mixture and component data as
    numpy arrays, and sys.stdout.writes the estimate of kappastars using the two gradient
    thresholds as detailed in the paper as KM1 and KM2"""

    sys.stdout.write('\nBegin Estimating Prior.This may take a long time, please be patient:)\n')
    sys.stdout.flush()
    X_mixture = np.asarray(X_mixture).reshape((int(num_mix), int(dim))).astype(np.float)
    X_component = np.asarray(X_component).reshape((int(num_com), int(dim))).astype(np.float)
    ...
    sys.stdout.write('Finish Estimating')
    sys.stdout.flush()
