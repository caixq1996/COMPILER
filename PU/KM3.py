...
def find_nearest_valid_distribution(u_alpha, kernel, initial=None, reg=0):
    ...
    # Test rank
    # PAG=np.concatenate((np.array(P),np.array(A),np.array(G)), axis=0)
    # if matrix_rank(PAG)!=n or matrix_rank(A)!=1:
...
def wrapper(X_mixture,X_component,num_mix,num_com,dim):
    ...
    X_mixture = np.array(X_mixture).reshape((int(num_mix),int(dim)))
    X_component = np.array(X_component).reshape((int(num_com),int(dim)))
    N=X_mixture.shape[0]
    M=X_component.shape[0]
    ...
    # # draw curve
    # print('Drawing curve...')
    # lambda_values=np.linspace(1,3,50)
    # dists = get_distance_curve(kernel, lambda_values, N=N, M=M)
    # # x_new = np.linspace(np.min(lambda_values), np.max(lambda_values), 50)
    # # y_smooth = make_interp_spline(lambda_values, dists)(x_new)
    # #plt.plot(x_new,y_smooth)
    # plt.plot(lambda_values,dists)
    # plt.xlabel('lambda')
    # plt.ylabel('d(\lambda)')
    # plt.savefig('./Figure/MPE_mimic.png')
    # #plt.show()
    ...
    # return (kappa_star_est_1,kappa_star_est_2)
    return kappa_star_est_1

#===============================code for MPE with RKME=========================
def mpe_RKME(kernel,N,M,w_mixture,w_RKME,nu,epsilon=0.04,lambda_upper_bound=8.):
    """ Do mixture proportion estimation (as in paper)for N  points from  
    mixture F and M points from component H, given kernel of size (N+M)x(N+M), 
    with first N points from  the mixture  and last M points from 
    the component, and return estimate of lambda_star where
    ...
    dist_diff = np.concatenate((w_mixture.reshape((N,1)) , -1 * w_RKME.reshape((M,1))))
    distribution_RKHS_distance = sqrt(np.dot(dist_diff.T, 
                                    np.dot(kernel, dist_diff))[0,0])
    lambda_left=1.
    lambda_right=lambda_upper_bound        
    while lambda_right-lambda_left>epsilon:
        lambda_value=(lambda_left+lambda_right)/2.        
        u_lambda = lambda_value * np.concatenate((w_mixture.reshape((N,1)),
            np.zeros((M, 1)))) + (1 - lambda_value) * \
            np.concatenate((np.zeros((N, 1)), w_RKME.reshape((M,1)) ))
        (solution, distance_sqd) = \
              find_nearest_valid_distribution(u_lambda, kernel)
        d_lambda_1=sqrt(distance_sqd)
...
def compute_best_rbf_kernel_width_RKME(X_mixture,X_RKME,w_mixture,w_RKME):        
    N=X_mixture.shape[0]
    M=X_RKME.shape[0]
    w_RKME = w_RKME / np.sum(w_RKME)
    w_mixture = w_mixture / np.sum(w_mixture)
    # compute median of pairwise distances
    X=np.concatenate((X_mixture,X_RKME))
    dot_prod_matrix=np.dot(X,X.T)
    norms_squared=sum(np.multiply(X,X).T)    
    distance_sqd_matrix=np.tile(norms_squared,(N+M,1)) + \
        np.tile(norms_squared,(N+M,1)).T - 2*dot_prod_matrix            
    ...
    for kernel_width in kernel_width_vals: 
        kernel=np.exp(-distance_sqd_matrix/(2.*kernel_width**2.))        
        dist_diff = np.concatenate((w_mixture.reshape((N,1)), 
                                    -1 * w_RKME.reshape((M,1)) ))
        distribution_RKHS_distance = sqrt(np.dot(dist_diff.T, 
                                        np.dot(kernel, dist_diff))[0,0])
        if distribution_RKHS_distance > max_dist_RKHS:
            max_dist_RKHS=distribution_RKHS_distance
...
def MPE_wrapper_RKME(X_mixture, X_RKME, num_mix, num_RKME, w_mixture, w_RKME, dim=1):
    """ Takes in 2 arrays containing the mixture data and the RKME for previous tasks as 
    numpy arrays, and prints the estimate of kappastars using the two gradient 
    thresholds as detailed in the paper as KM1 and KM2"""    
    X_mixture = np.array(X_mixture).reshape((int(num_mix),int(dim)))
    X_RKME = np.array(X_RKME).reshape((int(num_RKME),int(dim)))
    N = X_mixture.shape[0]    
    M = X_RKME.shape[0]
    best_width,kernel = compute_best_rbf_kernel_width_RKME(X_mixture,X_RKME,w_mixture, w_RKME)

    #Deciding threshold
    lambda_values = np.array([1.00,1.1])
    dists = get_distance_curve_RKME(kernel,lambda_values,w_mixture,w_RKME,N=N,M=M)
    begin_slope=(dists[1]-dists[0])/(lambda_values[1]-lambda_values[0])
    dist_diff = np.concatenate((w_mixture.reshape((N,1)), -1 * w_RKME.reshape((M,1))))
    distribution_RKHS_dist = sqrt(np.dot(dist_diff.T, np.dot(kernel, dist_diff))[0,0])
    thres_par=0.25
    ...
    #MPE
    lambda_star_est_1=mpe_RKME(kernel,N,M,w_mixture,w_RKME, nu=nu1)
    kappa_star_est_1=(lambda_star_est_1-1)/lambda_star_est_1
    ...
    #MPE
    # lambda_star_est_2=mpe_RKME(kernel,N,M,w_RKME,nu=nu2)
    # kappa_star_est_2=(lambda_star_est_2-1)/lambda_star_est_2
    # kappa_star_est_2 = 0
    return kappa_star_est_1
...
if __name__=='__main__':
    ...
    X_component = float(sys.argv[1])
    X_mixture = float(sys.argv[2])
    (KM2,KM1)=wrapper(X_mixture,X_component)
