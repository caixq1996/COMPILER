...
class DataGenerator:
    def __init__(self, ob_shape, data_dir, batch_size, traj_len, n_workers, preprocessing_offline):
        ...
        self.preprocessing_offline = preprocessing_offline

        data = tf.data.Dataset.from_tensor_slices(self.np_list)

        data = data.shuffle(len(self)).repeat()

        data = data.batch(1, drop_remainder=True)  # if tf version >= 1.10
    ...
    def _process_demonstrations(self, demonstrations_files):
        ...
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", demonstrations_files)

        r = np.load(demonstrations_files[0].decode('UTF-8'))
        low_reward_traj = r['data_low'].copy()
        np.random.shuffle(low_reward_traj)
        high_reward_traj = r['data_high'].copy()
        np.random.shuffle(high_reward_traj)
        ...
        if self.preprocessing_offline:
            low_reward_snippet = low_reward_traj.astype(np.float32, copy=False)
            high_reward_snippet = high_reward_traj.astype(np.float32, copy=False)
        else:
            # Convert int to float and normalise to [0.0, 1.0] range
            low_reward_snippet = low_reward_traj[: self.batch_size].astype(np.float32, copy=False)
            # low_reward_snippet = low_reward_snippet.astype(np.float32, copy=False)
            # low_reward_snippet /= 255.0
            high_reward_snippet = high_reward_traj[: self.batch_size].astype(np.float32, copy=False)
            # high_reward_snippet = high_reward_snippet.astype(np.float32, copy=False)
            # high_reward_snippet /= 255.0
        return low_reward_snippet, high_reward_snippet
