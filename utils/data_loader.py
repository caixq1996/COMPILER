...
class DataGenerator:
    def __init__(self, ob_shape, data_dir, batch_size, traj_len, n_workers, preprocessing_offline):
        ...
        # The data is still in the form of demonstrations, we need to preprocess and generate the trajectories live
            ...
            # data = data.map(lambda np_sample:
            #                 tf.py_func(self._process_demonstrations, [np_sample],
            #                            [tf.float32, tf.double]),
                            num_parallel_calls=n_workers)
            data = data.filter(lambda low_traj, high_traj, low_reward, high_reward: tf.not_equal(low_reward, high_reward))\
                .prefetch(buffer_size=5*batch_size)
